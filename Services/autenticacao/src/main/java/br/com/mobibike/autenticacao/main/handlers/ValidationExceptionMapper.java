package br.com.mobibike.autenticacao.main.handlers;
...
public class ValidationExceptionMapper implements ExceptionMapper<ConstraintViolationException>{
	...
	public Response toResponse(ConstraintViolationException constraintViolationExceptionException) {
		...
		return Response.status(BikeHttpStatusCode.CONTRATO_VIOLADO).type(MediaType.APPLICATION_JSON).entity(buildResponseMessage(constraintViolationExceptionException)).build();
	}
	...
	private List<MessageResponseError> buildResponseMessage(ConstraintViolationException constraintViolationException) {
		...
		for (ConstraintViolation<?> violation : constraintViolationException.getConstraintViolations()) {
			MessageResponseError messageResponseError = MessageResponseError.of("api.validation." + violation.getPropertyPath(), BikeHttpStatusCode.CONTRATO_VIOLADO, violation.getMessage());
